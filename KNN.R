...
#Caret package available models: http://topepo.github.io/caret/available-models.html
...
# where we used 'testData[,2:56]'. Your data set might have different numbers of predictors
...
testData <- read.csv("DatasetTest.csv", stringsAsFactor=TRUE, header=TRUE)

#list of all classifcation and regression methods supported by R
#names(getModelInfo())
grid <- expand.grid(k = seq(1,25))
...
model <- train(ID~., data=originalData, trControl=train_control, method="knn",metric="ROC", tuneGrid =grid)
model
plot(model)
